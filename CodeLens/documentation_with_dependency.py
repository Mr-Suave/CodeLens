# This file can be used for generating the dependency analysis along with generating the documentation (paid services issue :( ) Change the name of the file in the script.py to use this 
...
# Set your API Keys
OPENAI_API_KEY = "open_ai_api_key"  # For dependency analysis
GEMINI_API_KEY = "AIzaSyDvSZJk-8DI5CFv_9VPBDyfU_-ebGOUQ1g"  # For documentation generation

def query_openai(prompt, max_retries=3):
    """
    Send a request to OpenAI API for dependency analysis.
    ...
    while retries <= max_retries:
        try:
            response = openai.chat.completions.create(
                model="gpt-3.5-turbo",  # Using GPT-4 for comprehensive analysis
                messages=[
                    {"role": "system", "content": "You are an expert code analyst specializing in dependency analysis."},
            ...
            error_message = str(e)
            print(f"OpenAI API call failed: {error_message}")
            
            # Check if it's a rate limit error
            if "rate limit" in error_message.lower() or "429" in error_message:
                retry_seconds = 60  # Default retry time for rate limits
                print(f"Rate limit reached. Waiting for {retry_seconds} seconds before retrying...")
                time.sleep(retry_seconds)
                retries += 1
                continue
            
            # For other errors or if we've exhausted retries, return the error message
            if retries >= max_retries:
                return f"Failed to generate dependency analysis after {max_retries} retries. Error: {error_message}"
            
            # For non-rate-limit errors, retry after a short delay
...
def query_gemini(prompt, max_retries=3):
    """
    Send a request to Gemini API for documentation generation.
    Handles rate limit errors by waiting for the prescribed time and retrying.
    """
    retries = 0
    while retries <= max_retries:
        try:
            ...
            error_message = str(e)
            print(f"Gemini API call failed: {error_message}")
            
            # Check if it's a rate limit error (429)
            if "429" in error_message and "retry_delay" in error_message:
                # Extract the retry delay from the error message
                import re
                retry_seconds_match = re.search(r"seconds: (\d+)", error_message)
                if retry_seconds_match:
                    retry_seconds = int(retry_seconds_match.group(1))
                    print(f"Rate limit reached. Waiting for {retry_seconds} seconds before retrying...")
                    time.sleep(retry_seconds + 1)  # Add 1 extra second to be safe
                    retries += 1
                    continue
                else:
                    # If we can't parse the delay, use a default
                    print("Couldn't parse retry delay, waiting 60 seconds...")
                    time.sleep(60)
                    retries += 1
                    continue
            ...
            # For other errors or if we've exhausted retries, return the error message
            if retries >= max_retries:
                return f"Failed to generate documentation after {max_retries} retries. Error: {error_message}"
...
def extract_adjacency_list(function_list_path):
    ...
    if not os.path.exists(function_list_path):
        print(f"Error: Function list file not found at {function_list_path}. Please generate the function graph using the command codelens findbug")
        return [], {}
...
def generate_dependency_analysis(repo_path, user_type):
    ...
    Makes an API call to OpenAI for this analysis.
    """
...
"""
    ...
    print(f"Generating dependency analysis for {user_type} using OpenAI API...")
    start_time = time.time()
    dependency_analysis = query_openai(prompt)  # Use OpenAI API
    print(f"Dependency analysis generated in {time.time() - start_time:.2f} seconds")
    
    # Save the analysis to a file in the root directory
...
def generate_documentation(user_type, repo_path):
    ...
    wantDependency=input("Do you want to generate dependency analysis[y/n]:")
    if wantDependency=='y':
        # First, generate the dependency analysis with OpenAI API
        dependency_analysis = generate_dependency_analysis(repo_path, user_type)
        print("Waiting 9 seconds before making the next API call...")
        time.sleep(9)  # Brief pause between API calls
    else:
        dependency_analysis = "No dependency analysis requested."
    
    # Then proceed with the main documentation generation using Gemini
...
"""
    ...
    start_time = time.time()
    documentation_text = query_gemini(prompt)  # Use Gemini API for documentation
    print("Done")
